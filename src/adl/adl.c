...
// todo: use adl to manage temp

#if 0 /*defined(HAVE_ADL) &&*/ && (defined(__linux) || defined (WIN32))
...
/* debug flags */
static bool opt_debug = 1;
static bool opt_log_output = 0;
static bool opt_realquiet = 0;
static bool want_per_device_stats = 1;

/* global log_level, messages with lower or equal prio are logged */
static int opt_log_level = 0;

static const int opt_cutofftemp = 70;
static const int opt_autofan = 1;
static const int opt_autoengine = 1;
static const int opt_restart = 0;
static void app_restart(){}
...
bool gpu_stats(int gpu, float *temp, int *engineclock, int *memclock, float *vddc, int *activity, int *fanspeed, int *fanpercent, int *powertune);
	...
	if (engine && ga->autoengine) {
		if (temp > cgpu->cutofftemp) {
			...
			*denable = DEV_RECOVER;
			...
			applog(LOG_INFO, "Device recovered to temperature below target, re-enabling");
			*denable = DEV_ENABLED;
		}
